...
def orm(request):
    blog = models.Blog.objects.filter(site='vfast').select_related('user').first()
...
def index(request, *args, **kwargs):
    ...
    :param request:
    :param args:
    :param kwargs: article_type_id
    :return:
    '''
    # 文章类型列表article_type_list
    ...
    return render(request, 'index.html', {'article_list': article_list,  # 文章列表
                                          'aritcle_type_id': article_type_id,  # 文章id
                                          'article_type_list': article_type_list,  # 文章类型列表
                                          'page_str': page_str  # 分页
                                          }
                  )
...
def home(request, site):
    ...
    博主个人首页
    :param request:
    :param site:博主的网站后缀如：http://xxx.com/name.html
    :return:
    '''
    ...
    date_list = models.Article.objects.raw('select nid,count(nid) as num,strftime("%Y-%m",create_time) as ctime \
    from repository_article group by  strftime("%Y-%m",create_time)')
    for item in date_list:
        print(item.nid, item.num, item.ctime)
    article_list = models.Article.objects.filter(blog=blog).order_by('-nid').all()
    return render(request, 'home.html', {'blog': blog, 'tag_list': tag_list, 'category_list': category, \
...
def detail(request, site, nid):
    ...
    return render(request, 'home_detail.html',
                  {'blog': blog, 'article': article, 'comment_list': comment_list, 'tag_list': tag_list,
                   'category_list': category, 'date_list': date_list})
...
def filter(request, site, condition, val):
    ...
    if condition == 'tag':
        ...
        article_list = models.Article.objects.filter(category__title=val, blog=blog).all()
    elif condition == 'date':
